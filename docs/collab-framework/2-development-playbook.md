...
4. **判斷門檻**（§D）— 何時進下一 Phase / 插 reviewer / 派新 sub / 等使用者
5. **UI 慣例 / 任務類型→guide 對照**（§E + §F）— 升級自候選庫的規則、發包必讀的判斷依據
6. **源頭整合**（§G）— 本文件合併自 dispatch-prompt-handbook / multi-session-dispatch §七之二~三 + §8.11 / knowledge-validation-sop
...
- [ ] 對照 §F 任務類型 → guide 對照表，**逐條**加必讀（§B.0.1 ritual 見下）
- [ ] 發包 prompt 10 段齊：
   ...
   9. 測試位置
   10. 回報要求（4 段 + topic id + **含進 P1 前第一輪思考回饋，見 §B.0.5**）
...
> 本節記錄**已升級進規範正文**的 UI 慣例。新項從 `ref/candidate-precedents.md` 整理升級而來。

（當前空，待累積）

候選項例：
- 列表式 admin 頁面站點選擇用 `websiteCascade` 共用元件
- 金額顯示 `formatAmount` / 信用點整數用 `formatAmount(v, 0)` 千分位
- Modal 二次確認：單向操作（如 settle）用「同意書 checkbox + popconfirm」；雙向操作（如 transfer）只 popconfirm
- 無資料 / 無卡片引導：顯示提示 + 「關閉」或「去綁卡」連結
- 銀行卡號顯示遮罩 `****XXXX`（後四碼）

---

## §F 任務類型 → Guide 對照表（發包必讀查詢）

**原則**：有 guide 的就放必讀；沒 guide 的才讓 sub-session 從既有 code 摸。

（路徑省略 `docs/guides/` 前綴，以 Platform-BackendServer 為例）

| 任務類型 | 必讀 guide |
|---|---|
| 新建 table 模組 | `framework/tables-guides/`（README + 01-QUICK-START + 02-ENTITY-GUIDE + 03-BUILDER-API-REFERENCE）|
| 業務 Facade / domain service | `principles/architecture-guides/README.md` + 對應 domain guide |
| 交易鎖 | `framework/transaction-lock-guides/` |
| DbOperation 模式 | `framework/db-operation-guides/` |
| Ledger 寫入 | `framework/agent-ledger-guides/` |
| 排程 Job | `framework/scheduler-guides/` |
| Cache | `framework/cache-guides/` |
| 批次交易 | `framework/batch-transaction-pattern/` |
| Dynamic Fields | `framework/dynamic-fields-guides/` |
| Interceptor | `framework/interceptor-guides/` |
| Decorators | `framework/decorators-guides/` |
| Logger | `framework/logger-guides/` |
| i18n 翻譯 | `framework/i18n-guides/` |
| System Alert | `framework/system-alert-guides/` |
| **Admin 後台頁面（前後端）** | `domain/admin-guides/` + `domain/admin-component-guides/` + `framework/i18n-guides/` |
| 錢包操作 | `domain/wallet-guides/` |
| 優惠 | `domain/favour-guides/` |
| 統計 | `domain/statistics-guides/` |
| 代理佣金 | `domain/agent-commission-release-order-guides/` + 既有 `docs/audit/agent-commission_v2/` |
| 遊戲商介接 | `domain/gameprovider-guides/` |
| 支付 / 充值 | `domain/payment-guides/` + `domain/recharge-guides/` |
| 玩家交易記錄 | `domain/player-transaction-guides/` |
| 大廳 | `domain/lobby-guides/` |
| 風控 | `domain/risk-guides/` |
| 站點管理 | `domain/website-guides/` |
| **測試場景（e2e 類任務強制）** | `domain/test-scenarios-guides/` ⚠ 目錄結構規範（兩層 `{domain}/{feature}/` + `testScene-{NN}-{group}` 檔名）|

**組合判定**：看任務涉及哪些「能力」。Admin 頁面 + 新 table + 交易鎖 + ledger → 四項 guide 都放。

**本對照表為 Platform-BackendServer 特定** — 其他專案各自補自己的對照表在該專案的相應位置。
...
| `dispatch-prompt-handbook` 全 | §B.0 發包前 + §F 對照表 |
| `dispatch-prompt-handbook §附錄 A` sys_menu | §B.5 P7.1 |
| `dispatch-prompt-handbook §附錄 B` Excel 匯出 | §E UI 慣例（待整合）|
...
- v0.1 — 2026-04-21 建立初版（B.0-B.9 全 Phase / §C 交界送 sub / §D 門檻 / §F 對照表）
