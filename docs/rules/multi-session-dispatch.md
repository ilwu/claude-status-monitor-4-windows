...
| 主 session | 統籌、發包 prompt、review 回報、階段整合 | 最終業務拍板、大規模重構 |
| sub-session | 執行 Phase、測試、回報、遇阻請示 | 業務決策、擴大範圍、破壞性 git |

**最重要的邊界**：sub-session 遇**業務決策**必須停，不自推進。區分：
- **執行問題**（tsc 錯、路徑錯、小 typo）→ 自己解
- **業務問題**（流程異動、欄位語意、和規範衝突）→ 停 + `mmsg topic-add` 請示
